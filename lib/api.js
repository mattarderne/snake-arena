...
const API_BASE = (process.env.SNAKE_ARENA_API || "https://arena-web-vinext.matt-15d.workers.dev")
  .replace(/\/+$/, "");
const API_MODE = (process.env.SNAKE_ARENA_API_MODE || "web").trim().toLowerCase();
...
function useDirectBackend() {
  return API_MODE === "direct" || API_MODE === "backend" || API_MODE === "cloudflare" || API_MODE === "proxy";
}

function ensureDirectEndpointSupported(endpoint) {
  if (!useDirectBackend()) return;
  ...
  if (!supported.has(endpoint)) {
    throw new Error(
      `Endpoint '${endpoint}' is not supported by direct backend mode yet. `
      + "Use SNAKE_ARENA_API_MODE=web or point SNAKE_ARENA_API at the web frontend.",
    );
  }
}
...
function resolveUrl(endpoint, params = "") {
  if (useDirectBackend()) {
    ensureDirectEndpointSupported(endpoint);
