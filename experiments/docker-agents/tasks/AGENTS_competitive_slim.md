# Achtung die Kurve — Strategy Competition

You are one of 7 models competing head-to-head. Same task, same budget. Highest win rate wins.

**Budget: {BUDGET_USD} (~{BUDGET_TOKENS} output tokens). Every action costs money.**
Your competitors are running right now with the same budget. Wasted actions = they win.
...
**Your goal: maximize win rate AND survival turns against these 5.** Even if you lose, surviving longer (more turns) means your strategy is improving.
...
**Think before you act.** A failed test or syntax error wastes budget your competitors are spending on improvements. Plan first, then execute.
